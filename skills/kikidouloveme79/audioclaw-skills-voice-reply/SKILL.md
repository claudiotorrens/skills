...
description: Use when AudioClaw Skills, OpenClaw, PicoClaw, Feishu, or Lark needs to send SenseAudio voice replies with runtime-switchable voice_id, emotion preset, or speaking style, including per-message speaker overrides, voice-family emotion routing, cache reuse, and safe fallback when a requested voice is unavailable.
---
...
- A user has already said "以后一直给我发语音", and OpenClaw should remember that preference for future turns.
...
3. Run `scripts/openclaw_voice_switchboard.py`.
   - For PicoClaw on Feishu or Lark, prefer `scripts/picoclaw_voice_reply.py`.
...
6. If the result should be sent through AudioClaw media upload, pass:
   - `--out` inside the OpenClaw workspace
...
7. Use the returned JSON manifest in AudioClaw to:
   - prefer `scripts/picoclaw_voice_reply.py` for PicoClaw on Feishu
...
When this skill is used inside PicoClaw for Feishu or Lark voice replies:
...
4. Do not call the `message` tool with the local path or the `MEDIA:...` reference. PicoClaw will send them as plain text.
5. After the audio is sent, do not send an extra text confirmation such as `已发你了。` unless the user explicitly asked for a text reply too.
6. Use `media_reference` only as debug metadata or future OpenClaw compatibility data.

This rule matters because this PicoClaw environment does not render `MEDIA:...` as media, and the generic `send_file` tool sends Feishu voice notes as plain files instead of `audio` messages. The reliable path here is direct Feishu upload plus `msg_type=audio`.
...
- This skill requests final-file TTS in non-stream mode by default, because OpenClaw only needs the completed file and this avoids stream assembly edge cases.
...
4. The script returns a manifest with:
   ...
   - `trace_id`
5. AudioClaw uploads the resulting file to Feishu or any downstream channel, or lets the PicoClaw wrapper do that directly for Feishu.
...
- Force generated audio files to mode `0644` so the OpenClaw sender can read them reliably.
- When `--openclaw-workspace-root` is set and `--out` stays inside that root, expose `delivery.openclaw_media_reference`.
- When `--delivery-profile feishu_voice` is enabled, synthesize with SenseAudio first and then transcode to `ogg/opus` with system `ffmpeg` or `imageio-ffmpeg`.
...
- For PicoClaw on Feishu, `scripts/picoclaw_voice_reply.py` now uses the local Feishu app credentials from `~/.picoclaw/config.json`, uploads the audio through the official `/open-apis/im/v1/files` endpoint, and sends it as `msg_type=audio`.
...
Generate one OpenClaw turn from a JSON request:
...
python3 scripts/openclaw_voice_switchboard.py \
  ...
  --out ~/.openclaw/workspace/openclaw_warm.ogg
```

PicoClaw Feishu one-step example:
...
- `scripts/feishu_audio_sender.py`
  ...
  - Uses `~/.picoclaw/config.json` app credentials, infers the active chat, uploads the file, and sends `msg_type=audio`
