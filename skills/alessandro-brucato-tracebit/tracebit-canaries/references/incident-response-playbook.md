...
- Tool calls that read `~/.aws/credentials`, `~/.ssh/`, or `.env` files
...
|-------------|------------------------------|
| **aws** | AWS credentials were read and used. Likely: credential harvesting from `~/.aws/credentials` or `.env`, or prompt injection that caused an AWS API call. |
| **ssh** | SSH private key was read and used to attempt a connection. Likely: SSH key enumeration or exfiltration from `~/.ssh/`. |
