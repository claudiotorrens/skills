...
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${TESLA_COMMANDER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/tesla-commander}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
tesla-commander v$VERSION

Multi-purpose utility tool

Usage: tesla-commander <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "tesla-commander v$VERSION" ;;
