#!/usr/bin/env bash
# character-creator-cn - Chinese content creation tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${CHARACTER_CREATOR_CN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/character-creator-cn}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
character-creator-cn v$VERSION

Chinese content creation tool

Usage: character-creator-cn <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "character-creator-cn v$VERSION" ;;
