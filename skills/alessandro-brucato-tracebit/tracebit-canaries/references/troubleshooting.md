...
### AWS credentials not appearing in `~/.aws/credentials`

```bash
# Verify the file exists and has a [canary] section
cat ~/.aws/credentials | grep -A5 '\[canary\]'
...
### SSH key not appearing in `~/.ssh/`

```bash
# Check
ls -la ~/.ssh/tracebit-canary

# If missing:
tracebit deploy ssh

# Check permissions (SSH requires 600)
